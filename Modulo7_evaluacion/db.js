...
async function getData(n) {
  ...
  const cursor = client.query(
    new Cursor(
      "SELECT a.nombre, continente, poblacion, pib_2019, pib_2020 FROM paises a JOIN paises_pib b ON a.nombre=b.nombre"
    )
  );
  const respuesta = [];

  let fetchMoreRows = true;
  while (fetchMoreRows) {
    const rows = await cursor.read(n);
    respuesta.push(...rows);
    fetchMoreRows = rows.length === n;
  }
...
async function agregar(nombre, continente, poblacion, pib_2019, pib_2020) {
  const client = await pool.connect();
  try {
...
module.exports = { init, getData, agregar, eliminar, getAllData };
